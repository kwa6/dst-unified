#!/usr/bin/env bash
# train_llama_twostage.sh — Two-stage LoRA fine-tuning: (LUAS or D0T) → MultiWOZ
#
...
#
# Usage:
#   HF_TOKEN=hf_... bash scripts/train_llama_twostage.sh luas
#   HF_TOKEN=hf_... bash scripts/train_llama_twostage.sh d0t
#   HF_TOKEN=hf_... bash scripts/train_llama_twostage.sh luas meta-llama/Llama-3.2-3B-Instruct
#
# Arguments:
#   $1  Dataset for stage 1: 'luas' or 'd0t'  (required)
#   $2  Model                 (default: meta-llama/Llama-2-7b-chat-hf)
set -euo pipefail

if [ -z "${HF_TOKEN:-}" ]; then
    echo "Error: HF_TOKEN environment variable not set."
    echo "Get your token from: https://huggingface.co/settings/tokens"
    echo "Then run: export HF_TOKEN=hf_... && bash scripts/train_llama_twostage.sh luas"
    exit 1
fi
...
if [ -z "$DATASET" ]; then
    ...
    echo "Usage: bash scripts/train_llama_twostage.sh [luas|d0t] [model]"
    exit 1
fi
...
STAGE1_DIR="runs/llama_stage1_${DATASET}"
STAGE2_DIR="runs/llama_stage2_${DATASET}_mwoz_final"
...
echo
echo "============================================"
echo "TWO-STAGE LLAMA FINE-TUNING"
echo "============================================"
